...
func TestRequestIDAcceptsAndReturnsTraceID(t *testing.T) {
	...
	req.Header.Set("X-Trace-ID", "trace-from-browser")
	RequestID(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		if got := TraceID(r.Context()); got != "trace-from-browser" {
			t.Fatalf("trace id=%q", got)
		}
		Write(w, http.StatusOK, nil)
	})).ServeHTTP(rec, req)
	if rec.Header().Get("X-Trace-ID") != "trace-from-browser" {
		t.Fatalf("response trace=%q", rec.Header().Get("X-Trace-ID"))
	}
}
