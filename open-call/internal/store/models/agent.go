...
type AgentSession struct {
	// CurrentCallID 当前分配的通话，用于拒绝过期回调。
	CurrentCallID string `gorm:"comment:当前通话 ID"`
	// PendingCheckout 当前通话结束后签出。
	PendingCheckout bool `gorm:"not null;default:false;comment:等待签出"`
	// ID 签入会话 UUID。
...
// TableName 指定表名。
func (AgentSession) TableName() string { return "oc_agent_sessions" }
...
// TableName 指定表名。
func (AgentStateLog) TableName() string { return "oc_agent_state_log" }
...
// TableName 指定表名。
func (AgentSessionQueue) TableName() string { return "oc_agent_session_queues" }
