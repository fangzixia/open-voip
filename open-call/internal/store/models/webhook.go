...
// TableName 指定表名。
func (WebhookSubscription) TableName() string { return "oc_webhook_subscriptions" }
...
type WebhookDelivery struct {
	...
	// EventID 同一业务事件的稳定标识，与 subscription_id 共同保证幂等入队。
	EventID string `gorm:"type:uuid;not null;uniqueIndex:idx_oc_webhook_delivery_event_subscription"`
	// EventType 事件类型。
	EventType string `gorm:"size:64;not null;index;comment:事件类型"`
	// Payload 请求体 JSON。
	Payload string `gorm:"type:text;not null;comment:载荷 JSON"`
	// Status 投递状态：pending / processing / success / dead_letter。
	...
	// NextAttemptAt 后台任务下次可领取时间。
	NextAttemptAt time.Time `gorm:"not null;index;comment:下次尝试时间"`
	// LockedAt 当前投递租约开始时间。
	LockedAt *time.Time `gorm:"comment:任务租约时间"`
	// DeadLetterAt 达到最大重试次数的时间。
	DeadLetterAt *time.Time `gorm:"comment:进入死信时间"`
	// CreatedAt 创建时间。
	CreatedAt time.Time `gorm:"comment:创建时间"`
	// UpdatedAt 更新时间。
...
// TableName 指定表名。
func (WebhookDelivery) TableName() string { return "oc_webhook_deliveries" }
