...
- **强化**：一键/Compose 单机安装、内网 HTTPS、配置导出备份、本机磁盘与日志  
...
| ID | 需求描述 | 优先级 | 角色 | 验收要点 |
|----|----------|--------|------|----------|
| DEPLOY-01 | 提供 **单机一体化** 安装方式（如 Docker Compose 或单安装包），组件默认本机互联 | P0 | 管理员 | 一台干净内网机器按文档可启动全栈 |
...
| DEPLOY-04 | 数据持久化：数据库与录音目录挂载到宿主机；**升级镜像不丢数据** | P0 | 系统 | 重启/升级后 CDR 与录音仍在 |
...
| DEPLOY-07 | 可选 **离线/内网镜像** 部署说明（无法访问公网 Docker Hub 时） | P2 | 管理员 | 文档路径清晰 |
...
- **DEPLOY-01～04**：单机 Compose/安装、内网 HTTPS、数据持久化  
...
- PSTN、Webhook、质检、高级 IVR、班长强插、离线镜像包等  
...
| v0.2 | 2026-09-18 | 部署场景：小型公司内网单机；新增 DEPLOY-*，调整 TURN/容量/Out of Scope |
