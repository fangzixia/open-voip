...
func (u *sipUA) rebuildACL() {
	...
	if u.cfg.LocalRegistrar {
		nets = append(nets, parseAllowedNets([]string{"127.0.0.1/32", "::1/128"})...)
	}
	u.mu.Lock()
	u.allowed = nets
	u.mu.Unlock()
...
func (u *sipUA) onInvite(req *sip.Request, tx sip.ServerTransaction) {
	...
	got, _, err := u.onIn(context.Background(), did, from, callID)
	if err != nil || got == "" {
	...
	want100rel := require100rel(req.GetHeaders("Require")) || require100rel(req.GetHeaders("Supported"))
	if want100rel {
		sess.rseq = 1
	...
	if !u.waitWebRTC(sess, dlg, callID) {
		return
	}
...
func (u *sipUA) waitWebRTC(sess *sipSession, dlg *sipgo.DialogServerSession, callID string) bool {
	...
	for time.Now().Before(deadline) {
		...
		if u.media.hasWebRTCPeer(callID) {
			return true
		}
		select {
		case <-dlg.Context().Done():
...
func (u *sipUA) onRegister(req *sip.Request, tx sip.ServerTransaction) {
	src := req.Source()
	if !u.cfg.LocalRegistrar || !u.ipAllowed(src) {
...
func (u *sipUA) checkRegistrarAuth(req *sip.Request, tx sip.ServerTransaction, aor string) bool {
	pass := u.cfg.RegistrarPassword
	auth := req.GetHeader("Authorization")
	val := ""
	if auth != nil {
		val = auth.Value()
	}
	ok := verifyRegistrarDigest(val, "REGISTER", aor, pass, u.nonceValid)
	if ok {
		return true
	}
	stale := val != ""
...
func (u *sipUA) originate(ctx context.Context, callID, legID, dial, trunkID string) error {
	...
	for hop := 0; hop <= sipMaxRedirectHops; hop++ {
		...
		if code >= 300 && code < 400 {
			if uri, ok := redirectURI(dres.Res); ok {
				recipient = uri
